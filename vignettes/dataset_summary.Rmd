...
```{r}
data(london_attacks, package = "crimemappingdata")
```

Dataset of `r format(nrow(london_attacks), big.mark = ",")` fatal terrorist attacks in London from 2010 to 2018.
...
```{r}
data("sao_paulo_homicides", package = "crimemappingdata")
```

Dataset of `r format(nrow(sao_paulo_homicides), big.mark = ",")` homicides recorded by police in the immediate region of Sao Paulo, Brazil, between 2017 and 2022.
...
```{r}
data("tempe_opioid_calls", package = "crimemappingdata")
```

Dataset of `r format(nrow(tempe_opioid_calls), big.mark = ",")` calls to emergency services related to opioid overdoses in Tempe, Arizona, from 2017 to 2024.
...
```{r}
data(japan_prefectures, package = "crimemappingdata")
```

Simple Features (SF) object containing the boundaries of the prefectures of Japan.

```{r}
japan_prefectures_head <- sf::st_drop_geometry(japan_prefectures)
japan_prefectures_head$geometry <- "&lt;SF polygon&gt;"

japan_prefectures_head |>
  head(10) |>
  knitr::kable()
```
...
```{r}
data(sao_paulo_muni, package = "crimemappingdata")
```
...
```{r}
sao_paulo_muni_head <- sf::st_drop_geometry(sao_paulo_muni)
sao_paulo_muni_head$geometry <- "&lt;SF polygon&gt;"
