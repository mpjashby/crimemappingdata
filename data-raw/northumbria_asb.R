...
northumbria_asb <- temp_data_dir |>
  dir("northumbria-street.csv$", full.names = TRUE, recursive = TRUE) |>
  ...
  filter(
    ...
    falls_within == "Northumbria Police",
    year == 2022
  ) |>
  mutate(
  ...
  st_intersection(select(northumbria_districts, geometry)) |>
  st_drop_geometry() |>
  select(month, longitude, latitude, location, lsoa_code) |>
  write_tsv("inst/extdata/northumbria_asb.tab") |>
  glimpse()
...
# Source:
northumbria_wards <- read_sf("https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Wards_December_2022_Boundaries_GB_BGC/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson", quiet = FALSE) |>
  clean_names() |>
  filter(
    lad22nm %in% c(
      "Newcastle upon Tyne",
  ...
  write_sf("inst/extdata/northumbria_wards.gpkg") |>
  glimpse()
...
northumbria_ward_pop <- ward_pop_file |>
...
write_xlsx(
  ...
  "inst/extdata/northumbria_ward_pop.xlsx",
